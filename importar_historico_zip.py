...
def consolidar_veiculos(empresa_id, gravar):
    ...
    try:
        ...
        for placa, itens in grupos.items():
            itens.sort(
                key=lambda item: (
                    -len([v for v in viagens if v["veiculo_id"] == item["id"]]),
                    item["id"],
                )
            )
            principal = itens[0]
            ...
            for duplicado in itens[1:]:
                if gravar:
                    if (
                        motorista_principal is None
                        and duplicado["motorista_id"] is not None
                    ):
                        con.execute(
                            "UPDATE veiculos SET motorista_id=? WHERE id=?",
                            (duplicado["motorista_id"], principal["id"]),
                        )
...
def definir_codigos_veiculos(empresa_id, empresa, gravar):
    ...
    veiculos = sorted(
        db.listar_veiculos(empresa_id), key=lambda item: placa_chave(item["placa"])
    )
    if gravar:
        con = db.conectar()
        try:
            for posicao, veiculo in enumerate(veiculos, 1):
                con.execute(
                    "UPDATE veiculos SET codigo=? WHERE id=?",
                    (f"{prefixo}-{posicao:03d}", veiculo["id"]),
                )
            con.commit()
        finally:
            con.close()
...
def assinatura_viagem(empresa_id, motorista_id, veiculo_id, row):
    return (
        ...
        motorista_id,
        veiculo_id,
        valor(row, "data_inicio"),
        valor(row, "data_fim"),
        (valor(row, "origem") or "").strip().upper(),
        (valor(row, "destino") or "").strip().upper(),
...
def consolidar_motoristas(empresa_id, empresa, gravar):
    ...
    try:
        for nome, itens in grupos.items():
            ...
            itens.sort(
                key=lambda x: (
                    -len(
                        [
                            v
                            for v in db.listar_viagens(empresa_id)
                            if v["motorista_id"] == x["id"]
                        ]
                    ),
                    x["id"],
                )
            ...
            if gravar:
                con.execute(
                    "UPDATE motoristas SET nome=? WHERE id=?", (nome, principal["id"])
                )
            for duplicado in itens[1:]:
                if gravar:
                    con.execute(
                        "UPDATE viagens SET motorista_id=? WHERE motorista_id=?",
                        (principal["id"], duplicado["id"]),
                    )
                    con.execute(
                        "UPDATE veiculos SET motorista_id=? WHERE motorista_id=?",
                        (principal["id"], duplicado["id"]),
                    )
...
def motorista_destino(mapa, empresa_id, empresa, origem, gravar):
    ...
    ident = db.criar_motorista(
        nome=nome,
        codigo=None,
        cpf=valor(origem, "cpf"),
        telefone=valor(origem, "telefone"),
        email=valor(origem, "email"),
        cnh=valor(origem, "cnh"),
        categoria_cnh=valor(origem, "categoria_cnh"),
        validade_cnh=valor(origem, "validade_cnh"),
        status=valor(origem, "status", "Ativo") or "Ativo",
        observacoes=valor(origem, "observacoes"),
...
def veiculo_destino(mapa, empresa_id, origem, motorista_id, gravar):
    ...
    ident = db.criar_veiculo(
        placa=chave,
        codigo=valor(origem, "codigo"),
        descricao=valor(origem, "descricao"),
        marca=valor(origem, "marca"),
        ano=valor(origem, "ano"),
        tipo=valor(origem, "tipo"),
        quilometragem=valor(origem, "quilometragem", 0) or 0,
        status=valor(origem, "status", "Ativo") or "Ativo",
...
def definir_codigos(empresa_id, empresa, gravar):
    ...
    motoristas = sorted(
        db.listar_motoristas(empresa_id), key=lambda x: texto_chave(x["nome"])
    )
    if gravar:
        con = db.conectar()
        try:
            for posicao, motorista in enumerate(motoristas, 1):
                con.execute(
                    "UPDATE motoristas SET codigo=? WHERE id=?",
                    (f"{prefixo}-{posicao:03d}", motorista["id"]),
                )
            con.commit()
        finally:
            con.close()
...
def importar(arquivo_zip, gravar):
    with zipfile.ZipFile(arquivo_zip) as pacote, tempfile.TemporaryDirectory() as pasta:
        membro = next(
            (n for n in pacote.namelist() if n.endswith("/despesas.db")), None
        )
        if not membro:
        ...
        try:
            ...
            empresas_origem = {
                x["id"]: x["nome"]
                for x in origem.execute("SELECT id,nome FROM empresas")
            }
            resumo = {
                "viagens_importadas": 0,
                "viagens_existentes": 0,
                "despesas": 0,
                "cargas": 0,
            }
            for origem_empresa_id, origem_empresa in empresas_origem.items():
                ...
                mapa_veiculos = {
                    placa_chave(v["placa"]): v["id"]
                    for v in db.listar_veiculos(empresa_id)
                }
                motoristas_origem = {
                    x["id"]: x
                    for x in origem.execute(
                        "SELECT * FROM motoristas WHERE empresa_id=?",
                        (origem_empresa_id,),
                    )
                }
                veiculos_origem = {
                    x["id"]: x
                    for x in origem.execute(
                        "SELECT * FROM veiculos WHERE empresa_id=?",
                        (origem_empresa_id,),
                    )
                }
                existentes = {
                    assinatura_viagem(
                        empresa_id, v["motorista_id"], v["veiculo_id"], v
                    ): v["id"]
                    for v in db.listar_viagens(empresa_id)
                }
                for viagem in origem.execute(
                    "SELECT * FROM viagens WHERE empresa_id=? ORDER BY id",
                    (origem_empresa_id,),
                ):
                    motorista = motorista_destino(
                        ...
                        empresa_id,
                        empresa,
                        motoristas_origem[viagem["motorista_id"]],
                        gravar,
                    )
                    veiculo = veiculo_destino(
                        mapa_veiculos,
                        empresa_id,
                        veiculos_origem[viagem["veiculo_id"]],
                        motorista,
                        gravar,
                    )
                    ...
                    if destino_viagem:
                        ...
                        destino_viagem = db.criar_viagem(
                            ...
                            data_fim=valor(viagem, "data_fim"),
                            origem=valor(viagem, "origem"),
                            destino=valor(viagem, "destino"),
                            motivo=valor(viagem, "motivo"),
                            cliente_atividade=valor(viagem, "cliente_atividade"),
                            hodometro_inicio=valor(viagem, "hodometro_inicio", 0) or 0,
                            hodometro_fim=valor(viagem, "hodometro_fim"),
                            media_computador_bordo=valor(
                                viagem, "media_computador_bordo"
                            ),
                            valor_adiantamento=valor(viagem, "valor_adiantamento", 0)
                            or 0,
                            valor_devolvido=valor(viagem, "valor_devolvido", 0) or 0,
                            valor_nf_ida=valor(viagem, "valor_nf_ida", 0) or 0,
                            valor_nf_retorno=valor(viagem, "valor_nf_retorno", 0) or 0,
                            status=valor(viagem, "status", "Finalizada")
                            or "Finalizada",
                            observacoes=valor(viagem, "observacoes"),
                        )
                        ...
                        resumo["viagens_importadas"] += 1
                        continue
                    for despesa in origem.execute(
                        "SELECT * FROM despesas WHERE viagem_id=?", (viagem["id"],)
                    ):
                        ja_existe = db._one(
                            "SELECT 1 FROM despesas WHERE viagem_id=? AND categoria=? AND data=? AND valor=? AND COALESCE(descricao,'')=COALESCE(?, '')",
                            (
                                destino_viagem,
                                despesa["categoria"],
                                despesa["data"],
                        ...
                        if not ja_existe and gravar:
                            db.criar_despesa(
                                viagem_id=destino_viagem,
                                categoria=despesa["categoria"],
                                data=despesa["data"],
                                valor=despesa["valor"],
                                litros=valor(despesa, "litros"),
                                local_abastecimento=valor(
                                    despesa, "local_abastecimento"
                                ),
                                forma_pagamento=valor(despesa, "forma_pagamento"),
                                descricao=valor(despesa, "descricao"),
                                estabelecimento=valor(despesa, "estabelecimento"),
                                quilometragem=valor(despesa, "quilometragem"),
                                status=valor(despesa, "status", "Pendente")
                                or "Pendente",
                            )
                            resumo["despesas"] += 1
                    for carga in origem.execute(
                        "SELECT * FROM cargas WHERE viagem_id=?", (viagem["id"],)
                    ):
                        ja_existe = db._one(
                            "SELECT 1 FROM cargas WHERE viagem_id=? AND empresa_cliente=? AND tipo=? AND data=? AND valor=?",
                            (
                                destino_viagem,
                                carga["empresa_cliente"],
                                carga["tipo"],
                                carga["data"],
                                carga["valor"],
                            ),
                        ...
                        if not ja_existe and gravar:
                            db.criar_carga(
                                viagem_id=destino_viagem,
                                empresa_cliente=carga["empresa_cliente"],
                                tipo=carga["tipo"],
                                data=carga["data"],
                                valor=carga["valor"],
                                descricao=valor(carga, "descricao"),
                            )
                            resumo["cargas"] += 1
                resumo[f"codigos_{empresa}"] = definir_codigos(
                    empresa_id, empresa, gravar
                )
                resumo[f"codigos_veiculos_{empresa}"] = definir_codigos_veiculos(
                    empresa_id, empresa, gravar
                )
            return resumo
        finally:
...
if __name__ == "__main__":
    ...
    if not db.USANDO_POSTGRES:
        raise SystemExit(
            "Defina DATABASE_URL do Neon antes de importar. Nada foi alterado."
        )
    resultado = importar(argumentos.arquivo_zip, gravar=not argumentos.dry_run)
    print(
        ("Simulação concluída:" if argumentos.dry_run else "Importação concluída:"),
        resultado,
    )
