...
TABELAS = [
    "empresas", "usuarios", "usuario_empresas", "motoristas", "veiculos",
    "viagens", "despesas", "anexos_despesa", "pagamentos",
    "categorias_despesa", "auditoria", "cargas",
]


def main():
    if not URL.startswith(("postgres://", "postgresql://")):
        raise SystemExit("Defina DATABASE_URL com a URL de conexão do Neon antes de executar.")
    ...
    try:
        with destino.cursor() as cur:
            cur.execute("TRUNCATE TABLE " + ", ".join(TABELAS) + " RESTART IDENTITY CASCADE")
            ...
            for tabela in TABELAS:
                cur.execute(
                    ...
                    (tabela,),
                ) if tabela not in ("usuario_empresas",) else None
        destino.commit()
