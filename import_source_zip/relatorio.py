...
def gerar_relatorio(empresa_id: int = None, caminho_saida: str = None) -> str:
    ...
    despesas_todas = db.listar_despesas()
    cargas_todas = db.listar_cargas()
    ...
    colunas1 = ["Empresa", "Motorista", "Veiculo", "Viagem", "Categoria", "Data", "Valor (R$)", "Litros", "Local Abastecimento", "Forma Pagamento", "Descricao"]
    ...
    for d in despesas_todas:
        ...
        ws1.cell(row=linha, column=2, value=fmt_codigo(v["motorista_nome"], v["motorista_codigo"]))
        ws1.cell(row=linha, column=3, value=fmt_codigo(v["veiculo_placa"], v["veiculo_codigo"]))
        ws1.cell(row=linha, column=4, value=f"#{v['id']} ({v['origem'] or ''} -> {v['destino'] or ''})")
        ws1.cell(row=linha, column=5, value=d["categoria"])
        ws1.cell(row=linha, column=6, value=fmt_data(d["data"]))
        ws1.cell(row=linha, column=7, value=round(d["valor"], 2))
        ws1.cell(row=linha, column=8, value=round(d["litros"], 2) if d["litros"] else None)
        ws1.cell(row=linha, column=9, value="Assis" if d["local_abastecimento"] == "ASSIS" else ("Viagem" if d["local_abastecimento"] == "VIAGEM" else ""))
        ws1.cell(row=linha, column=10, value="Dinheiro" if d["forma_pagamento"] == "DINHEIRO" else ("Cartao" if d["forma_pagamento"] == "CARTAO" else ""))
    ...
    colunas1b = ["Empresa (frota)", "Motorista", "Veiculo", "Viagem", "Empresa Atendida",
                 "Tipo", "Data", "Valor (R$)", "Descricao"]
    ...
    for c in cargas_todas:
        ...
        ws1b.cell(row=linha, column=2, value=fmt_codigo(v["motorista_nome"], v["motorista_codigo"]))
        ws1b.cell(row=linha, column=3, value=fmt_codigo(v["veiculo_placa"], v["veiculo_codigo"]))
        ws1b.cell(row=linha, column=4, value=f"#{v['id']} ({v['origem'] or ''} -> {v['destino'] or ''})")
        ws1b.cell(row=linha, column=5, value=c["empresa_cliente"])
        ws1b.cell(row=linha, column=6, value="Entrega" if c["tipo"] == "ENTREGA" else "Coleta")
    ...
    colunas2 = [
        "Viagem", "Empresa", "Motorista", "Veiculo", "Data Inicio", "Data Fim",
        "Km Rodado", "Litros (notas)", "Consumo Real (km/L)", "Media Painel (km/L)",
        "Diferenca % (consumo)", "Custo Medio Litro (R$)",
        "Preco Medio Assis (R$)", "Preco Medio Estrada (R$)", "Diferenca % (preco)",
        "Diarias", "Refeicoes", "Chapa", "Combustivel", "Arla", "Pecas", "Sinistros",
        "Pedagio", "Borracharia", "Freteiro", "Hospedagem", "Uber", "Outros",
        "Qtd Sinistros", "Total Geral (R$)",
        "Total Entrega (R$)", "Total Coleta (R$)", "Receita Total (R$)",
        "Custo sobre Receita (%)",
        "Adiantamento (R$)", "Devolvido (R$)", "Saldo a Prestar Contas (R$)",
        "Feedback Consumo", "Feedback Preco Combustivel", "Feedback Adiantamento",
    ]
    _cabecalho(ws2, colunas2)
    linha = 2
    for v in viagens:
        despesas_viagem = [d for d in despesas_todas if d["viagem_id"] == v["id"]]
        cargas_viagem = [c for c in cargas_todas if c["viagem_id"] == v["id"]]
        a = analisar_viagem(v, despesas_viagem, cargas_viagem)
        tot = a["totais_por_categoria"]
        valores = [
            v["id"], v["empresa_nome"], fmt_codigo(v["motorista_nome"], v["motorista_codigo"]), fmt_codigo(v["veiculo_placa"], v["veiculo_codigo"]),
            fmt_data(v["data_inicio"]), fmt_data(v["data_fim"]) or "(em aberto)",
            ...
            round(a["custo_medio_litro"], 2) if a["custo_medio_litro"] is not None else None,
            round(a["preco_medio_assis"], 2) if a["preco_medio_assis"] is not None else None,
            round(a["preco_medio_estrada"], 2) if a["preco_medio_estrada"] is not None else None,
            round(a["diferenca_preco_pct"], 1) if a["diferenca_preco_pct"] is not None else None,
            ...
            round(a["percentual_custo_receita"], 3) if a["percentual_custo_receita"] is not None else None,
        ...
        for col, val in enumerate(valores, 1):
            ...
            if col == col_feedback_consumo:
                ...
                if abs(a["saldo_adiantamento"]) >= 0.01:
                    ...
                    cel.font = FONTE_OK
        linha += 1
    _autofit(ws2, [8, 12, 18, 10, 12, 12, 10, 12, 16, 16, 14, 16, 16, 16, 14,
    ...
    colunas3 = ["Viagem", "Empresa", "Motorista", "Veiculo", "Km Rodado", "Litros (notas)",
                "Consumo Real (km/L)", "Media Painel (km/L)", "Diferenca %", "Feedback"]
    ...
    for v in viagens:
        ...
        valores = [
            v["id"], v["empresa_nome"], fmt_codigo(v["motorista_nome"], v["motorista_codigo"]), fmt_codigo(v["veiculo_placa"], v["veiculo_codigo"]),
            round(a["km_rodado"], 1), round(a["total_litros"], 2),
            round(a["consumo_real"], 2), round(a["media_painel"], 2),
            round(a["diferenca_pct"], 1), a["feedback"],
        ]
        for col, val in enumerate(valores, 1):
    ...
    if linha == 2:
        ws3.cell(row=2, column=1, value="Nenhum alerta -- todas as viagens com dados suficientes conferem com o painel.")

    # --- Aba 4: Alertas de Preco Combustivel ---
    ws4 = wb.create_sheet("Alertas Preco Combustivel")
    colunas4 = ["Viagem", "Empresa", "Motorista", "Veiculo", "Preco Medio Assis (R$)",
                "Preco Medio Estrada (R$)", "Diferenca %", "Feedback"]
    ...
    for v in viagens:
        ...
        valores = [
            v["id"], v["empresa_nome"], fmt_codigo(v["motorista_nome"], v["motorista_codigo"]), fmt_codigo(v["veiculo_placa"], v["veiculo_codigo"]),
            round(a["preco_medio_assis"], 2), round(a["preco_medio_estrada"], 2),
            round(a["diferenca_preco_pct"], 1), a["feedback_preco"],
        ]
        for col, val in enumerate(valores, 1):
    ...
    if linha == 2:
        ws4.cell(row=2, column=1, value="Nenhum alerta -- o preco pago na estrada ficou dentro do praticado em Assis em todas as viagens.")

    # --- Aba 5: Fechamento Mensal (entrega/coleta por empresa atendida) ---
    ws5 = wb.create_sheet("Fechamento Mensal")
    colunas5 = ["Mes", "Empresa Atendida", "Entrega (R$)", "Coleta (R$)", "Receita (R$)",
                "Despesa Alocada (R$)", "% Despesa sobre Receita", "Total Despesa Frete do Mes (R$)"]
    ...
    meses = sorted({d["data"][:7] for d in despesas_escopo} | {c["data"][:7] for c in cargas_escopo})
    for mes in meses:
        ...
        for emp, dd in sorted(m["por_empresa"].items(), key=lambda x: -x[1]["total"]):
            ...
            ws5.cell(row=linha, column=6,
                     value=round(dd["despesa_alocada"], 2) if dd["despesa_alocada"] is not None else None)
            ws5.cell(row=linha, column=7,
                     value=round(dd["percentual_despesa_sobre_receita"], 3) if dd["percentual_despesa_sobre_receita"] is not None else None)
            ws5.cell(row=linha, column=8, value=round(m["total_despesa_mes"], 2))
            linha += 1
    ...
    colunas6 = ["Mes", "Tipo", "Veiculo/Motorista", "Km Rodado", "Litros (notas)", "Consumo Medio (km/L)", "Qtd Viagens"]
    ...
    for mes in meses_viagens:
        ...
        viagens_com_despesas = [
            (v, [d for d in despesas_todas if d["viagem_id"] == v["id"]]) for v in viagens_mes
        ]
        ...
        for placa, d in sorted(consumo["por_veiculo"].items()):
            ...
            ws6.cell(row=linha, column=6, value=round(d["consumo_medio"], 2) if d["consumo_medio"] is not None else None)
    ...
    if linha == 2:
        ws6.cell(row=2, column=1, value="Nenhuma viagem com hodometro final e litros lancados ainda.")

    if caminho_saida is None:
        base = os.path.dirname(os.path.abspath(__file__))
        nome = f"relatorio_despesas_viagem_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        if empresa_id is not None:
            empresa = next((e for e in db.listar_empresas() if e["id"] == empresa_id), None)
