...
BASE_DIR=os.path.dirname(os.path.abspath(__file__)); DB_PATH=os.path.join(BASE_DIR,"despesas.db")
DATABASE_URL=os.environ.get("DATABASE_URL", "")
USANDO_POSTGRES=DATABASE_URL.startswith(("postgres://", "postgresql://"))
CATEGORIAS=["Combustível","Pedágio","Alimentação","Hospedagem","Estacionamento","Manutenção","Lavagem","Transporte","Outras"]
...
PERFIS=["Administrador","Gestor","Financeiro","Motorista","Usuário"]
...
def conectar():
 if USANDO_POSTGRES:return _ConexaoPostgres()
 c=sqlite3.connect(DB_PATH);c.row_factory=sqlite3.Row;c.execute("PRAGMA foreign_keys=ON");return c
def _insert_id(q,p):
 c=conectar()
 if USANDO_POSTGRES:
  row=c.execute(q+" RETURNING id",p).fetchone();i=row["id"]
 else:i=c.execute(q,p).lastrowid
...
def verificar_senha(p,h):
 try:return hmac.compare_digest(_hash(p,h.split('$',1)[0]),h)
 except (AttributeError,IndexError):return False
def _rows(q,p=()):
 c=conectar();r=c.execute(q,p).fetchall();c.close();return r
def _one(q,p=()):
 r=_rows(q,p);return r[0] if r else None

def inicializar():
 if USANDO_POSTGRES:
  _inicializar_postgres();return
 ...
 mig={"empresas":{"razao_social":"TEXT","cnpj":"TEXT","ativa":"INTEGER DEFAULT 1"},"motoristas":{"cpf":"TEXT","telefone":"TEXT","email":"TEXT","cnh":"TEXT","categoria_cnh":"TEXT","validade_cnh":"TEXT","status":"TEXT DEFAULT 'Ativo'","observacoes":"TEXT"},"veiculos":{"marca":"TEXT","ano":"INTEGER","tipo":"TEXT","quilometragem":"REAL DEFAULT 0","status":"TEXT DEFAULT 'Ativo'","motorista_id":"INTEGER"},"viagens":{"motivo":"TEXT","cliente_atividade":"TEXT","status":"TEXT DEFAULT 'Planejada'"},"despesas":{"estabelecimento":"TEXT","quilometragem":"REAL","status":"TEXT DEFAULT 'Pendente'","motivo_reprovacao":"TEXT","criado_por":"INTEGER","criado_em":"TEXT"}}
 for t,cols in mig.items():
  old={x[1] for x in c.execute(f"PRAGMA table_info({t})")}
  for n,k in cols.items():
   if n not in old:c.execute(f"ALTER TABLE {t} ADD COLUMN {n} {k}")
 c.execute("UPDATE empresas SET nome='ERIMAX',razao_social='Erimar Indústria e Comércio de Produtos para Saúde Ltda.',cnpj='11.463.608/0001-79' WHERE nome='ERIMAR'")
 c.execute("UPDATE empresas SET razao_social='Comercial Mark Atacadista Ltda.',cnpj='09.315.996/0001-07' WHERE nome='MARK'")
 c.execute("UPDATE empresas SET razao_social='Erimar Indústria e Comércio de Produtos para Saúde Ltda.',cnpj='11.463.608/0001-79' WHERE nome='ERIMAX'")
 for n,rj,cnpj in [("MARK","Comercial Mark Atacadista Ltda.","09.315.996/0001-07"),("ERIMAX","Erimar Indústria e Comércio de Produtos para Saúde Ltda.","11.463.608/0001-79")]:c.execute("INSERT OR IGNORE INTO empresas(nome,razao_social,cnpj) VALUES(?,?,?)",(n,rj,cnpj))
 for n in CATEGORIAS:c.execute("INSERT OR IGNORE INTO categorias_despesa(empresa_id,nome) VALUES(NULL,?)",(n,))
 if not c.execute("SELECT 1 FROM usuarios WHERE email='admin@viagens.local'").fetchone():
  u=c.execute("INSERT INTO usuarios(nome,email,senha_hash,perfil,criado_em) VALUES(?,?,?,?,?)",("Administrador","admin@viagens.local",_hash("admin123"),"Administrador",datetime.now().isoformat())).lastrowid
  for e in c.execute("SELECT id FROM empresas"):c.execute("INSERT INTO usuario_empresas VALUES(?,?)",(u,e[0]))
 c.commit();c.close()
...
def _inicializar_postgres():
 ...
 for n,rj,cnpj in [("MARK","Comercial Mark Atacadista Ltda.","09.315.996/0001-07"),("ERIMAX","Erimar Indústria e Comércio de Produtos para Saúde Ltda.","11.463.608/0001-79")]:c.execute("INSERT INTO empresas(nome,razao_social,cnpj) VALUES(%s,%s,%s) ON CONFLICT (nome) DO NOTHING",(n,rj,cnpj))
 for n in CATEGORIAS:
  if not c.execute("SELECT 1 FROM categorias_despesa WHERE empresa_id IS NULL AND nome=%s",(n,)).fetchone():c.execute("INSERT INTO categorias_despesa(empresa_id,nome) VALUES(NULL,%s)",(n,))
 if not c.execute("SELECT 1 FROM usuarios WHERE email=%s",("admin@viagens.local",)).fetchone():
  u=c.execute("INSERT INTO usuarios(nome,email,senha_hash,perfil,criado_em) VALUES(%s,%s,%s,%s,%s) RETURNING id",("Administrador","admin@viagens.local",_hash("admin123"),"Administrador",datetime.now().isoformat())).fetchone()["id"]
  for e in c.execute("SELECT id FROM empresas").fetchall():c.execute("INSERT INTO usuario_empresas VALUES(%s,%s)",(u,e["id"]))
 c.commit();c.close()

def autenticar(email,senha):
 u=_one("SELECT * FROM usuarios WHERE lower(email)=lower(?) AND ativo=1",(email.strip(),));return u if u and verificar_senha(senha,u['senha_hash']) else None
def listar_empresas(usuario_id=None):
 q="SELECT * FROM empresas WHERE ativa=1";p=[]
 if usuario_id:q+=" AND id IN(SELECT empresa_id FROM usuario_empresas WHERE usuario_id=?)";p=[usuario_id]
 return _rows(q+" ORDER BY nome",p)
def usuario_tem_empresa(u,e):return bool(_one("SELECT 1 FROM usuario_empresas WHERE usuario_id=? AND empresa_id=?",(u,e)))
def listar_usuarios():
 agregador="string_agg(e.nome, ', ')" if USANDO_POSTGRES else "group_concat(e.nome,', ')"
 return _rows(f"SELECT u.*,{agregador} empresas FROM usuarios u LEFT JOIN usuario_empresas ue ON ue.usuario_id=u.id LEFT JOIN empresas e ON e.id=ue.empresa_id GROUP BY u.id ORDER BY u.nome")
def criar_usuario(nome,email,senha,perfil,empresas):
 c=conectar();q="INSERT INTO usuarios(nome,email,senha_hash,perfil,criado_em) VALUES(?,?,?,?,?)";cur=c.execute(q+(" RETURNING id" if USANDO_POSTGRES else ""),(nome,email.lower(),_hash(senha),perfil,datetime.now().isoformat()));i=cur.fetchone()["id"] if USANDO_POSTGRES else cur.lastrowid
 for e in empresas:c.execute("INSERT INTO usuario_empresas VALUES(?,?)",(i,e))
 c.commit();c.close();return i
def alterar_senha(i,senha):
 c=conectar();c.execute("UPDATE usuarios SET senha_hash=? WHERE id=?",(_hash(senha),i));c.commit();c.close()
def auditar(u,e,a,ent,r=None):
 c=conectar();c.execute("INSERT INTO auditoria(usuario_id,empresa_id,acao,entidade,registro_id) VALUES(?,?,?,?,?)",(u,e,a,ent,r));c.commit();c.close()

def listar_motoristas(e):return _rows("SELECT * FROM motoristas WHERE empresa_id=? ORDER BY nome",(e,))
def criar_motorista(**d):
 return _insert_id("INSERT INTO motoristas(nome,codigo,cpf,telefone,email,cnh,categoria_cnh,validade_cnh,status,observacoes,empresa_id) VALUES(:nome,:codigo,:cpf,:telefone,:email,:cnh,:categoria_cnh,:validade_cnh,:status,:observacoes,:empresa_id)",d)
def listar_veiculos(e):return _rows("SELECT v.*,m.nome motorista_nome FROM veiculos v LEFT JOIN motoristas m ON m.id=v.motorista_id WHERE v.empresa_id=? ORDER BY placa",(e,))
def criar_veiculo(**d):
...
def obter_viagem(i):return _one("SELECT v.*,m.nome motorista_nome,ve.placa veiculo_placa FROM viagens v JOIN motoristas m ON m.id=v.motorista_id JOIN veiculos ve ON ve.id=v.veiculo_id WHERE v.id=?",(i,))
def criar_despesa(**d):
 for k,v in {"estabelecimento":None,"quilometragem":None,"status":"Pendente","criado_por":None,"litros":None,"forma_pagamento":None,"local_abastecimento":None}.items():d.setdefault(k,v)
 return _insert_id("INSERT INTO despesas(viagem_id,categoria,data,valor,litros,local_abastecimento,forma_pagamento,descricao,estabelecimento,quilometragem,status,criado_por) VALUES(:viagem_id,:categoria,:data,:valor,:litros,:local_abastecimento,:forma_pagamento,:descricao,:estabelecimento,:quilometragem,:status,:criado_por)",d)
def listar_despesas(viagem_id=None,empresa_id=None,status=None):
 q="SELECT d.*,v.empresa_id,v.id viagem_numero,m.nome motorista_nome,e.nome empresa_nome FROM despesas d JOIN viagens v ON v.id=d.viagem_id JOIN motoristas m ON m.id=v.motorista_id JOIN empresas e ON e.id=v.empresa_id WHERE 1=1";p=[]
 if viagem_id:q+=" AND d.viagem_id=?";p.append(viagem_id)
...
def anexar_comprovante(despesa_id,nome,caminho,tamanho,usuario_id):
 c=conectar();c.execute("INSERT INTO anexos_despesa(despesa_id,nome,caminho,tamanho,usuario_id) VALUES(?,?,?,?,?)",(despesa_id,nome,caminho,tamanho,usuario_id));c.commit();c.close()
def listar_anexos(despesa_id):return _rows("SELECT * FROM anexos_despesa WHERE despesa_id=? ORDER BY criado_em DESC",(despesa_id,))
def registrar_pagamento(i,data,valor,forma,u,obs):
 if not _one("SELECT 1 FROM despesas WHERE id=? AND status='Aprovada'",(i,)):raise ValueError("Somente despesas aprovadas podem ser pagas.")
 c=conectar();c.execute("INSERT INTO pagamentos(despesa_id,data,valor,forma_pagamento,responsavel_id,observacao) VALUES(?,?,?,?,?,?)",(i,data,valor,forma,u,obs));c.execute("UPDATE despesas SET status='Paga' WHERE id=?",(i,));c.commit();c.close()
def resumo_dashboard(e):
 ds=listar_despesas(empresa_id=e);vs=listar_viagens(e);ms=listar_motoristas(e);f=lambda s:sum(float(x['valor']) for x in ds if x['status']==s);total=f('Aprovada')+f('Paga');return dict(total=total,pendente=f('Pendente'),aprovada=total,reprovada=f('Reprovada'),viagens=len(vs),motoristas=len([m for m in ms if m['status']=='Ativo']),media_viagem=total/len(vs) if vs else 0,media_motorista=total/len(ms) if ms else 0,despesas=ds)

def excluir_motorista(i):
 if _one("SELECT 1 FROM viagens WHERE motorista_id=?",(i,)): raise ValueError("Este motorista possui viagens registradas; marque-o como inativo para preservar o histórico.")
 c=conectar();c.execute("DELETE FROM motoristas WHERE id=?",(i,));c.commit();c.close()
